...
let INPUTS: {[name: string]: InputFeature} = {
  ...
  "absX": {f: (x, y) => Math.abs(x), label: "|X_1|"},
  "absY": {f: (x, y) => Math.abs(y), label: "|X_2|"},
  "xCubed": {f: (x, y) => x * x * x, label: "X_1^3"},
  "yCubed": {f: (x, y) => y * y * y, label: "X_2^3"},
  "expX": {f: (x, y) => Math.exp(x)/100, label: "exp(X_1)"},
  "expY": {f: (x, y) => Math.exp(y)/100, label: "exp(X_2)"},
  "sigmoidX": {f: (x, y) => 1/(1+Math.exp(-x)), label: "σ(X_1)"},
  "sigmoidY": {f: (x, y) => 1/(1+Math.exp(-y)), label: "σ(X_2)"},
};
...
// 决策边界历史记录
let boundaryHistory: {data: number[][], iter: number, lossTrain: number, lossTest: number}[] = [];
let isReplaying = false;
let replayIter = 0;
let replayTimerIndex = 0;
...
function makeGUI() {
  ...
  // 回放控制事件
  d3.select("#replay-start-button").on("click", () => {
    startReplay();
  });
  
  d3.select("#replay-stop-button").on("click", () => {
    stopReplay();
  });
  
  d3.select("#replay-slider").on("input", function() {
    if (isReplaying) {
      stopReplay();
    }
    let index = parseInt(this.value);
    loadBoundaryFromHistory(index);
  });
  
  // 隐藏层激活展示控制
  d3.select("#show-activation").on("change", function() {
    showActivation = this.checked;
    if (network) {
      updateActivationUI(network);
    }
  });
}
...
// 更新隐藏层激活状态UI
function updateActivationUI(network: nn.Node[][]) {
  // 如果不显示激活状态，就用默认颜色
  if (!showActivation) {
    for (let layerIdx = 1; layerIdx < network.length - 1; layerIdx++) {
      ...
      for (let i = 0; i < layer.length; i++) {
        let node = layer[i];
        d3.select(`rect#node-rect-${node.id}`)
          .style("fill", "white");
      }
    }
    return;
  ...
  // 计算每个隐藏层节点的平均激活值（在一些样本点上）
  // 我们在整个输入空间均匀采样一些点
  let samples = 20; // 减少采样点数量，避免影响性能
  let nodeActivations: {[nodeId: string]: number[]} = {};
  ...
  for (let layerIdx = 1; layerIdx < network.length - 1; layerIdx++) {
    ...
    for (let i = 0; i < layer.length; i++) {
      let node = layer[i];
      nodeActivations[node.id] = [];
    }
  }

  // 采样点并计算激活值
  let xScale = d3.scale.linear().domain([0, samples - 1]).range(xDomain);
  let yScale = d3.scale.linear().domain([samples - 1, 0]).range(xDomain);
    ...
    for (let j = 0; j < samples; j++) {
      ...
      for (let layerIdx = 1; layerIdx < network.length - 1; layerIdx++) {
        ...
        for (let k = 0; k < layer.length; k++) {
          let node = layer[k];
          nodeActivations[node.id].push(node.output);
        }
      }
    }
  }
  ...
  for (let layerIdx = 1; layerIdx < network.length - 1; layerIdx++) {
    ...
    for (let i = 0; i < layer.length; i++) {
      let node = layer[i];
      let activations = nodeActivations[node.id];
      let avgActivation = d3.mean(activations) || 0;

      // 将激活值映射到颜色（使用类似的colorScale）
      // 但是我们用透明度和颜色深浅来表示
      d3.select(`rect#node-rect-${node.id}`)
        .style("fill", colorScale(avgActivation))
        .style("opacity", 0.7);
    }
  }
}
...
function updateUI(firstStep = false) {
  ...
  // Update hidden layer activation visualization (only every 5 steps or first step).
  if (firstStep || iter % 5 === 0) {
    updateActivationUI(network);
  }
  ...
  // 保存决策边界历史（每5次保存一次，避免内存过大）
  if (!isReplaying && iter % 5 === 0 && firstStep === false) {
    saveBoundaryToHistory();
  }
}
...
function reset(onStartup=false) {
  ...
  // 停止回放但不尝试恢复UI（network还没重建）
  isReplaying = false;
  replayTimerIndex++; 
  resetHistory();

  d3.select("#layers-label").text("隐藏层").attr("title", "神经网络中间的处理层，负责提取和转换特征。层数越多网络越复杂，能学习更复杂模式但也更容易过拟合。");
...
// 保存当前决策边界到历史
function saveBoundaryToHistory() {
  // 获取输出节点的ID
  let outputNodeId = nn.getOutputNode(network).id;
  
  // 深拷贝输出节点的边界数据
  let dataCopy: number[][] = [];
  let data = boundary[outputNodeId];
  ...
  // 更新回放进度条最大值
  let slider = d3.select("#replay-slider").node() as HTMLInputElement;
  if (slider) {
    slider.max = (boundaryHistory.length - 1).toString();
  }
}

// 从历史中加载特定的决策边界
function loadBoundaryFromHistory(index: number) {
  if (index < 0 || index >= boundaryHistory.length) {
    return;
  }
  
  let historyItem = boundaryHistory[index];
  
  // 更新背景
  heatMap.updateBackground(historyItem.data, state.discretize);
  
  // 更新损失显示
  d3.select("#loss-train").text(historyItem.lossTrain.toFixed(3));
  d3.select("#loss-test").text(historyItem.lossTest.toFixed(3));
  
  let pad = "000000";
  let iterStr = (pad + historyItem.iter).slice(-pad.length);
  iterStr = iterStr.replace(/\B(?=(\d{3})+(?!\d))/g, ",");
  d3.select("#iter-number").text(iterStr);
  
  // 更新进度条显示
  d3.select("#replay-iter").text(historyItem.iter.toString());
}

// 开始回放
function startReplay() {
  if (boundaryHistory.length === 0) {
    alert("还没有训练历史，请先训练网络！");
    return;
  }
  ...
  let localTimerIndex = ++replayTimerIndex;
  let lastUpdate = Date.now();
  
  d3.timer(() => {
    if (localTimerIndex !== replayTimerIndex) {
      return true; // 停止
    }
    ...
    if (now - lastUpdate >= 100) {
      ...
      if (replayIter >= boundaryHistory.length) {
        stopReplay();
        return true;
      }
      
      loadBoundaryFromHistory(replayIter);
      
      // 更新进度条
      let slider = d3.select("#replay-slider").node() as HTMLInputElement;
      if (slider) {
        slider.value = replayIter.toString();
      }
...
function stopReplay() {
  isReplaying = false;
  replayTimerIndex++; // 让所有现有定时器停止
  
  // 恢复到当前状态 - 只更新决策边界和显示信息，不重新计算
  if (network) {
    ...
    // 更新损失显示和迭代数
    let pad = "000000";
    let iterStr = (pad + iter).slice(-pad.length);
    iterStr = iterStr.replace(/\B(?=(\d{3})+(?!\d))/g, ",");
